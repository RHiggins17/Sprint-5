...
function parseTransform(jsonNode, sceneNode)
{
	...
	if ("translate" in jsonNode) {
		...
		sceneNode.__dirtyPosition = true;
	}
...
function parseObject(jsonNode, sceneNode)
{
    ...
    var onTraverse = function (child) {
	    ...
	    //if (child instanceof Physijs.ConvexMesh) {
			//alert(child.geometry.name);
			//geometry = child.geometry;
			//child = new Physijs.PlaneMesh(geometry, material);
...
function parseMesh(jsonNode)
{
	...
	if (geometryType == "cube") {
		...
		if ("mass"   in jsonNode) mass   = jsonNode["mass"];
		geometry = new THREE.BoxGeometry(width, height, depth);
		mesh = new Physijs.BoxMesh(geometry, material, mass);
		//mesh.addEventListener('collision');
		// set position
		//if ("translate" in jsonNode) translate = jsonNode["translate"];
		...
		mesh = new Physijs.SphereMesh(geometry, material, mass);
		mesh.addEventListener( 'collision');
		// set position
		//if ("translate" in jsonNode) translate = jsonNode["translate"];
		//mesh.position.set(translate[0], translate[1], translate[2]);
		...
		mesh = new Physijs.CylinderMesh(geometry, material, mass);
		mesh.addEventListener( 'collision');
		// set position
		//if ("translate" in jsonNode) translate = jsonNode["translate"];
		//mesh.position.set(translate[0], translate[1], translate[2]);
		...
		mesh = new Physijs.PlaneMesh(geometry, material);
		mesh.addEventListener( 'collision');
		// set position
		//if ("translate" in jsonNode) translate = jsonNode["translate"];
		//mesh.position.set(translate[0], translate[1], translate[2]);
		...
		mesh = new Physijs.ConeMesh(geometry, material, mass);
		mesh.addEventListener( 'collision');
		// set position
		//if ("translate" in jsonNode) translate = jsonNode["translate"];
		//mesh.position.set(translate[0], translate[1], translate[2]);
...
function parseMaterial(jsonNode)
{
	...
	if (type == "meshLambertMaterial")
	{
		...
		physijsMaterial = Physijs.createMaterial(material, .8, .4);
